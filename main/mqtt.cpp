...
void handleOtaCmd(const char* data, int data_len)
{
  ...
  sscanf(data, "%8s", cmd);
    ...
    constexpr size_t SHA256_LENGTH = 64;
    char *sha256 = (char*)malloc( SHA256_LENGTH+1 );
    if (1 == sscanf(data + strlen(STR_CMD_START), "%64s", sha256)) {
      xTaskCreate(&otaTask, "otaTask", 8192, (void*)sha256, 3, &otaTaskHandle);
    } else {
      ESP_LOGE(TAG, "ota start expects the SHA256 sum");
    }
	} else if (strncasecmp(cmd, "reboot", CMD_LENGTH) ==0) { }
}
...
void mqttTask(void*)
{
	...
	for (;;) {
		...
		snprintf(topic, TOPIC_LEN, MQTT_PREFIX "/%s",
				mqttEvent.name);
